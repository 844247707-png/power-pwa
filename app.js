...
const plan = [
  ...
  {day:3,title:'全身训练 B',minutes:35,exercises:[['后撤弓步','每侧 8 次','3 组'],['斜板俯卧撑','8–12 次','3 组'],['臀桥','每侧 8 次','2 组'],['Dead Bug','每侧 8 次','2 组'],['侧平板','每侧 20–30 秒','2 组'],['平板支撑','30–45 秒','2 组']]},
  {day:4,title:'主动恢复',minutes:25,exercises:[['步行','30 分钟','1 组'],['猫牛式','8–10 次','1 组'],['轻拉伸','6 分钟','1 组']]},
  {day:5,title:'全身训练 A+',minutes:35,exercises:[['徒手深蹲','10–12 次','3 组'],['斜板俯卧撑','8–12 次','3 组'],['臀桥','12 次','3 组'],['Bird Dog','每侧 8 次','2 组'],['平板支撑','30–45 秒','3 组']]},
  {day:6,title:'轻活动 + 核心',minutes:30,exercises:[['步行','30–45 分钟','1 组'],['Bird Dog','每侧 8 次','2 组'],['Dead Bug','每侧 8 次','2 组'],['臀桥','12 次','2 组'],['平板支撑','30 秒','2 组']]},
...
const exerciseLibrary = [
  ...
  {name:'婴儿式', category:'拉伸', visual:'child_pose', tags:['拉伸','恢复'], videoQuery:'child pose stretch tutorial', videoLabel:'婴儿式视频', summary:'训练后舒缓背部和肩部的经典动作。', points:['臀部坐向脚后跟。','手臂向前伸，肩放松。'], errors:['耸肩过紧。'], fix:['只要有轻拉伸感即可。'], breath:'缓慢深呼吸。'},
  {name:'腹部伸展', category:'拉伸', visual:'cobra', tags:['拉伸','腹部'], videoQuery:'cobra stretch tutorial', videoLabel:'腹部伸展视频', summary:'做完平板支撑后非常适合放松前侧。', points:['趴地后用手轻轻撑起上半身。','骨盆尽量贴地。'], errors:['强压腰部。'], fix:['腰不舒服就减小高度。'], breath:'撑起时自然呼吸。'},
  {name:'大腿前侧拉伸', category:'拉伸', visual:'quad_stretch', tags:['拉伸','腿部'], videoQuery:'quadriceps stretch standing tutorial', videoLabel:'股四头肌拉伸视频', summary:'放松股四头肌。', points:['扶墙站稳。','同侧手抓脚踝向臀部靠近。'], errors:['膝盖外张。'], fix:['两侧膝盖尽量并拢。'], breath:'自然呼吸。'},
  {name:'大腿后侧拉伸', category:'拉伸', visual:'hamstring_stretch', tags:['拉伸','腿后侧'], videoQuery:'hamstring stretch seated tutorial', videoLabel:'腿后侧拉伸视频', summary:'放松腿后侧和髋后链。', points:['一腿伸直，一腿弯曲。','从髋部向前折叠。'], errors:['强行弓背摸脚尖。'], fix:['保持脊柱自然，不要硬压。'], breath:'自然呼吸。'}
];
...
function videoSrc(query){return `https://www.youtube.com/embed?listType=search&list=${encodeURIComponent(query)}&autoplay=0&playsinline=1`;}
function searchHref(query){return `https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`;}
...
$('#settingsBtn').addEventListener('click', ()=>{route='settings'; render();});
  ...
  return `
    <section class="hero">
      ...
      <div class="day-switch">
        <button class="day-btn secondary-day" onclick="goPrevDay()">← 前一天</button>
        <button class="day-btn" onclick="goNextDay(false)">后一天 →</button>
      </div>
  ...
  return `
    ...
    <div class="card soft">
      ...
      <div class="action-row mt12">
        <button class="ghost-btn compact" onclick="goPrevDay()">← 前一天</button>
        <button class="ghost-btn compact" onclick="goNextDay(false)">后一天 →</button>
      </div>
...
function completeSet(){
  ...
  if(training.set < totalSets){ training.set++; showCountdown(75, ()=>render(), '组间休息'); }
    ...
    showModal(`<div class="kicker">WORKOUT COMPLETE</div><h2>训练完成</h2><p class="note">建议现在进行约 8 分钟舒缓拉伸。</p><button class="primary" onclick="closeModal();startStretch()">开始拉伸</button><button class="secondary" onclick="closeModal();render()">稍后再做</button>`)
  }
}
...
function renderStretch(){
  ...
  $('#view').innerHTML=`
    <div class="exercise-hero">
      ...
      <p class="note">${s[2]}</p>
      <button class="secondary" onclick="showExerciseByKeyword('${s[0].replace(/·.*/,"")}')">查看动作视频</button>
...
function nextStretch(){ clearInterval(timerId); if(stretchSession.index < stretches.length-1){ stretchSession.index++; stretchSession.left=stretches[stretchSession.index][1]; stretchSession.running=false; renderStretch(); } else{ stretchSession=null; showModal(`<div class="kicker">RECOVERY COMPLETE</div><h2>拉伸完成</h2><p class="note">今天的训练流程结束。补水并正常进食即可。</p><button class="primary" onclick="closeModal();render()">完成</button>`); }}

function foodView(){
  const labels=['早餐','午餐','加餐','晚餐'];
  return `
  <div class="card"><div class="row"><div><div class="kicker">DAILY TARGET</div><div class="metric">${state.profile.calorieGoal}<span style="font-size:14px"> kcal</span></div></div><div style="text-align:right"><div class="metric">${state.profile.proteinGoal}<span style="font-size:14px"> g</span></div><div class="metric-label">蛋白质</div></div></div></div>
  <div class="section-title"><h2>Day ${state.day} 饮食</h2><span class="pill">外卖可执行</span></div>
  <div class="list">${mealsByDay[state.day].map((x,i)=>`<div class="list-item ${state.mealDone[i]?'done':''}" onclick="toggleMeal(${i})"><div><strong>${labels[i]}</strong><div class="mini">${x}</div></div><div class="check">${state.mealDone[i]?'✓':''}</div></div>`).join('')}</div>
  <div class="section-title"><h2>快速记录</h2></div>
  <div class="card"><div class="form-row"><div><label>增加热量 kcal</label><input class="inline-input" id="addCal" type="number" placeholder="如 650"></div><div><label>增加蛋白质 g</label><input class="inline-input" id="addPro" type="number" placeholder="如 42"></div></div><label style="margin-top:12px">饮水 L</label><input class="inline-input" id="addWater" type="number" step=".1" placeholder="如 0.5"><button class="primary" onclick="addIntake()">加入今日数据</button></div>`;
}
...
function swapVisual(visual, phase){ const box=$('#exerciseVisualBox'); if(box) box.innerHTML=exerciseSVG(visual,phase); }
...
function dataView(){
  ...
  return `
    ...
    <div class="card"><div class="row"><div><div class="kicker">WEIGHT TREND</div><h3>近 7 次记录</h3></div></div><div class="chart">${logs.map(x=>`<div class="bar" style="height:${x.weight? 30+((x.weight-min)/(max-min||1))*100:8}px"></div>`).join('')}</div></div>
    <div class="card"><h3>记录身体数据</h3><div class="form-row mt16"><div><label>体重 kg</label><input id="logWeight" class="inline-input" type="number" step=".1" value="${lastLog()?.weight??state.profile.weight}"></div><div><label>腰围 cm</label><input id="logWaist" class="inline-input" type="number" step=".1" value="${lastLog()?.waist??''}"></div></div><label style="margin-top:12px">昨晚睡眠 h</label><input id="logSleep" class="inline-input" type="number" step=".1" value="${lastLog()?.sleep??''}"><button class="primary" onclick="saveLog()">保存记录</button></div>`;
}
...
function settingsView(){
  return `
    <div class="section-title"><h2>设置</h2><span class="pill">${APP_VERSION}</span></div>
    <div class="card"><div class="form-row"><div><label>身高 cm</label><input id="pHeight" class="inline-input" type="number" value="${state.profile.height}"></div><div><label>体重 kg</label><input id="pWeight" class="inline-input" type="number" step=".1" value="${state.profile.weight}"></div><div><label>每日热量 kcal</label><input id="pCal" class="inline-input" type="number" value="${state.profile.calorieGoal}"></div><div><label>蛋白质 g</label><input id="pPro" class="inline-input" type="number" value="${state.profile.proteinGoal}"></div></div><label style="margin-top:12px">每日饮水 L</label><input id="pWater" class="inline-input" type="number" step=".1" value="${state.profile.waterGoal}"><button class="primary" onclick="saveProfile()">保存目标</button></div>
    <div class="card"><div class="row wrap"><div><div class="kicker">PROGRAM</div><h3>当前：Day ${state.day}</h3></div><div class="action-row"><button class="ghost-btn compact" onclick="goPrevDay()">前一天</button><button class="ghost-btn compact" onclick="goNextDay(false)">后一天</button></div></div><p class="note">现在可以自由前后切换，不会因为误触进入下一天后无法返回。</p></div>
    <div class="card"><h3>数据备份</h3><button class="secondary" onclick="exportData()">导出 JSON</button><button class="secondary" onclick="document.getElementById('importFile').click()">导入 JSON</button><input id="importFile" type="file" accept="application/json" style="display:none" onchange="importData(event)"></div>
    <div class="card"><h3>缓存与版本</h3><p class="note">本版本已改为更稳妥的缓存策略。如果你看到旧页面，可以点下面按钮清理旧缓存并重新加载。</p><button class="secondary" onclick="hardRefresh()">清理缓存并刷新</button></div>
  `;
}
...
async function hardRefresh(){ try{ if('caches' in window){ const keys = await caches.keys(); await Promise.all(keys.map(k=>caches.delete(k))); } localStorage.setItem('power_force_refresh', Date.now()); location.href = './?v=' + Date.now(); }catch(e){ location.reload(true); } }

function goPrevDay(){ state.day = state.day<=1 ? 7 : state.day-1; save(); render(); }
function goNextDay(skipConfirm){ if(!skipConfirm && !state.trainDone){ if(!confirm('今天训练还没有完成，仍然进入后一天吗？\n你之后仍然可以随时返回前一天。')) return; } state.day = state.day>=7 ? 1 : state.day+1; save(); render(); }

function exerciseSVG(type, phase='start'){
  const s = phase==='start';
  switch(type){
    case 'squat': return svgWrap(`<line x1="60" y1="42" x2="60" y2="82" ${stroke()}/><circle cx="60" cy="26" r="10" ${stroke()}/><line x1="60" y1="52" x2="38" y2="62" ${stroke()}/><line x1="60" y1="52" x2="82" y2="62" ${stroke()}/>${s? `<line x1="60" y1="82" x2="46" y2="118" ${stroke()}/><line x1="60" y1="82" x2="74" y2="118" ${stroke()}/>` : `<line x1="60" y1="82" x2="44" y2="100" ${stroke()}/><line x1="44" y1="100" x2="32" y2="118" ${stroke()}/><line x1="60" y1="82" x2="76" y2="100" ${stroke()}/><line x1="76" y1="100" x2="88" y2="118" ${stroke()}/>`}<line x1="24" y1="120" x2="96" y2="120" stroke="#bbb" stroke-width="3" stroke-linecap="round"/>`);
    case 'incline_pushup': return svgWrap(`<line x1="90" y1="40" x2="90" y2="120" stroke="#cfcfcf" stroke-width="4"/><line x1="90" y1="80" x2="65" y2="${s?72:84}" ${stroke()}/><circle cx="55" cy="${s?68:80}" r="8" ${stroke()}/><line x1="65" y1="${s?72:84}" x2="35" y2="${s?92:96}" ${stroke()}/><line x1="35" y1="${s?92:96}" x2="18" y2="104" ${stroke()}/><line x1="67" y1="${s?74:86}" x2="83" y2="84" ${stroke()}/><line x1="63" y1="${s?74:86}" x2="82" y2="93" ${stroke()}/>`);
    case 'glute_bridge': return svgWrap(`<line x1="15" y1="120" x2="105" y2="120" stroke="#bbb" stroke-width="3" stroke-linecap="round"/><circle cx="28" cy="${s?96:82}" r="9" ${stroke()}/><line x1="36" y1="${s?98:84}" x2="${s?55:58}" y2="${s?106:86}" ${stroke()}/><line x1="${s?55:58}" y1="${s?106:86}" x2="${s?70:80}" y2="${s?112:88}" ${stroke()}/><line x1="${s?70:80}" y1="${s?112:88}" x2="${s?88:100}" y2="${s?104:98}" ${stroke()}/><line x1="${s?88:100}" y1="${s?104:98}" x2="95" y2="120" ${stroke()}/><line x1="${s?78:88}" y1="${s?110:90}" x2="75" y2="120" ${stroke()}/>`);
    case 'bird_dog': return svgWrap(`<line x1="40" y1="70" x2="70" y2="70" ${stroke()}/><circle cx="30" cy="68" r="8" ${stroke()}/><line x1="42" y1="72" x2="${s?52:20}" y2="${s?100:68}" ${stroke()}/><line x1="68" y1="72" x2="80" y2="100" ${stroke()}/><line x1="50" y1="70" x2="${s?76:96}" y2="${s?48:70}" ${stroke()}/><line x1="66" y1="70" x2="${s?52:20}" y2="${s?100:102}" ${stroke()}/><line x1="18" y1="104" x2="88" y2="104" stroke="#bbb" stroke-width="3" stroke-linecap="round"/>`);
    case 'plank': return svgWrap(`<line x1="15" y1="108" x2="105" y2="108" stroke="#bbb" stroke-width="3" stroke-linecap="round"/><circle cx="30" cy="${s?66:74}" r="8" ${stroke()}/><line x1="38" y1="${s?68:76}" x2="84" y2="${s?78:70}" ${stroke()}/><line x1="48" y1="78" x2="42" y2="108" ${stroke()}/><line x1="82" y1="${s?78:70}" x2="90" y2="108" ${stroke()}/>`);
    case 'reverse_lunge': return svgWrap(`<circle cx="60" cy="26" r="10" ${stroke()}/><line x1="60" y1="40" x2="60" y2="80" ${stroke()}/><line x1="60" y1="52" x2="42" y2="62" ${stroke()}/><line x1="60" y1="52" x2="78" y2="62" ${stroke()}/>${s? `<line x1="60" y1="80" x2="50" y2="118" ${stroke()}/><line x1="60" y1="80" x2="76" y2="118" ${stroke()}/>` : `<line x1="60" y1="80" x2="48" y2="108" ${stroke()}/><line x1="48" y1="108" x2="44" y2="118" ${stroke()}/><line x1="60" y1="80" x2="82" y2="98" ${stroke()}/><line x1="82" y1="98" x2="96" y2="118" ${stroke()}/>`}<line x1="28" y1="120" x2="100" y2="120" stroke="#bbb" stroke-width="3" stroke-linecap="round"/>`);
    case 'dead_bug': return svgWrap(`<circle cx="24" cy="60" r="8" ${stroke()}/><line x1="32" y1="60" x2="62" y2="60" ${stroke()}/><line x1="46" y1="60" x2="${s?40:76}" y2="${s?36:36}" ${stroke()}/><line x1="46" y1="60" x2="${s?40:76}" y2="${s?84:84}" ${stroke()}/><line x1="62" y1="60" x2="${s?84:98}" y2="${s?40:60}" ${stroke()}/><line x1="62" y1="60" x2="${s?84:98}" y2="${s?80:60}" ${stroke()}/><line x1="10" y1="100" x2="100" y2="100" stroke="#bbb" stroke-width="3" stroke-linecap="round"/>`);
    case 'side_plank': return svgWrap(`<line x1="20" y1="108" x2="100" y2="108" stroke="#bbb" stroke-width="3" stroke-linecap="round"/><circle cx="${s?34:40}" cy="${s?54:72}" r="8" ${stroke()}/><line x1="${s?40:46}" y1="${s?58:76}" x2="82" y2="${s?92:84}" ${stroke()}/><line x1="50" y1="74" x2="40" y2="108" ${stroke()}/><line x1="82" y1="${s?92:84}" x2="90" y2="108" ${stroke()}/><line x1="${s?52:48}" y1="${s?74:76}" x2="${s?56:64}" y2="${s?46:52}" ${stroke()}/>`);
    case 'child_pose': return svgWrap(`<line x1="18" y1="110" x2="100" y2="110" stroke="#bbb" stroke-width="3" stroke-linecap="round"/><circle cx="${s?48:32}" cy="${s?84:80}" r="8" ${stroke()}/><line x1="${s?56:40}" y1="${s?86:82}" x2="${s?76:60}" y2="${s?96:86}" ${stroke()}/><line x1="${s?76:60}" y1="${s?96:86}" x2="90" y2="102" ${stroke()}/><line x1="${s?76:60}" y1="${s?96:86}" x2="24" y2="102" ${stroke()}/>`);
    case 'cobra': return svgWrap(`<line x1="18" y1="110" x2="100" y2="110" stroke="#bbb" stroke-width="3" stroke-linecap="round"/><circle cx="${s?26:48}" cy="${s?90:58}" r="8" ${stroke()}/><line x1="${s?34:56}" y1="${s?92:62}" x2="${s?64:80}" y2="${s?98:84}" ${stroke()}/><line x1="${s?64:80}" y1="${s?98:84}" x2="92" y2="102" ${stroke()}/><line x1="60" y1="98" x2="52" y2="110" ${stroke()}/><line x1="72" y1="100" x2="64" y2="110" ${stroke()}/>`);
    case 'quad_stretch': return svgWrap(`<line x1="22" y1="120" x2="98" y2="120" stroke="#bbb" stroke-width="3" stroke-linecap="round"/><circle cx="58" cy="26" r="10" ${stroke()}/><line x1="58" y1="40" x2="58" y2="80" ${stroke()}/><line x1="58" y1="52" x2="40" y2="62" ${stroke()}/><line x1="58" y1="52" x2="78" y2="${s?62:50}" ${stroke()}/><line x1="58" y1="80" x2="50" y2="120" ${stroke()}/><line x1="58" y1="80" x2="78" y2="104" ${stroke()}/><line x1="78" y1="104" x2="${s?72:66}" y2="84" ${stroke()}/>`);
    case 'hamstring_stretch': return svgWrap(`<line x1="18" y1="120" x2="100" y2="120" stroke="#bbb" stroke-width="3" stroke-linecap="round"/><circle cx="${s?48:42}" cy="${s?56:52}" r="8" ${stroke()}/><line x1="${s?56:50}" y1="${s?60:56}" x2="60" y2="80" ${stroke()}/><line x1="60" y1="80" x2="90" y2="120" ${stroke()}/><line x1="60" y1="80" x2="28" y2="120" ${stroke()}/><line x1="${s?50:44}" y1="${s?64:58}" x2="82" y2="92" ${stroke()}/>`);
...
function render(){
  ...
  const view=$('#view');
  view.innerHTML = route==='home' ? homeView() : route==='train' ? trainView() : route==='library' ? libraryView() : route==='food' ? foodView() : route==='data' ? dataView() : settingsView();
  if(route!=='settings') document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.route===route));
  else document.querySelectorAll('.nav-item').forEach(b=>b.classList.remove('active'));
}
render();
